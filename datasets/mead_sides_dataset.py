...
def get_datasets_MEAD_sides(config=None):
    mead_sides_cfg = get_dataset_section(config, 'MEAD_sides')
    ...
    if not os.path.exists("assets/MEAD_lists.pkl"):
        ...
        for view in ['videos_left_30', 'videos_left_60', 'videos_right_30', 'videos_right_60']:
            ...
            files = [f for f in os.listdir(os.path.join(mead_sides_cfg.MEAD_sides_path,view))]

            files = [f for f in files if f.endswith('.mp4') and "test" not in f]

            for file in files:
                if file.split('_')[0] in train_subjects:
                    folder_path = os.path.join(mead_sides_cfg.MEAD_sides_path,view,file.split(".")[0]+".mp4")
                    landmarks_path = os.path.join(mead_sides_cfg.MEAD_sides_path,view,file.split(".")[0]+".npy")
