...
from src.base_trainer import BaseTrainer, BaseHairTrainer
import numpy as np
...
class SmirkHairTrainer(BaseHairTrainer):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
    ...
    def step1(self, batch):
        ...
        encoder_output = self.smirk_encoder(batch['img'], batch['hairmask'], batch['bodymask'])
        
        with torch.no_grad():       # base_encoder = smirk_encoder -> later used to regularize the weights
    ...
    # ---------------- second path ---------------- #
    def step2(self, encoder_output, batch, batch_idx, phase='train'):
        raise NotImplementedError("SmirkHairStepTrainer: step 2 not implemented")
        B, C, H, W = batch['img'].shape        
        img = batch['img']
        masks = batch['mask']
    ...
    def freeze_encoder(self):
        utils.freeze_module(self.smirk_encoder.strand_encoder, 'strand encoder')
        utils.freeze_module(self.smirk_encoder.depth_encoder, 'depth encoder')
        
    def unfreeze_encoder(self):
        if self.config.train.optimize_strand:
            utils.unfreeze_module(self.smirk_encoder.strand_encoder, 'strand encoder')
        
        if self.config.train.optimize_depth:
            utils.unfreeze_module(self.smirk_encoder.depth_encoder, 'depth encoder')
    ...
    def step(self, batch, batch_idx, phase='train'):
        ...
        if (self.config.train.loss_weights['cycle_loss'] > 0) and (phase == 'train'):
            raise NotImplementedError("SmirkHairStep: cycle_loss not implemented")
