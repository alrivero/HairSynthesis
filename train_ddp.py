...
def ensure_config_defaults(conf):
    ...
    if 'train' in conf and 'ddp' in conf.train:
        if 'find_unused_parameters' not in conf.train.ddp:
            conf.train.ddp.find_unused_parameters = True
...
def init_distributed():
    ...
    if missing:
        raise RuntimeError(
            "train_ddp.py must be launched with torchrun; missing env vars: "
            + ', '.join(missing)
        )
...
def maybe_convert_sync_batchnorm(trainer, config):
    ...
    if getattr(config.arch, 'enable_fuse_generator', False):
        trainer.smirk_generator = torch.nn.SyncBatchNorm.convert_sync_batchnorm(trainer.smirk_generator)
    return trainer


def wrap_trainable_modules(trainer, local_rank, config):
    ddp_cfg = getattr(config.train, 'ddp', None)
    find_unused_parameters = bool(cfg_get(ddp_cfg, 'find_unused_parameters', True))
    ...
    if getattr(config.arch, 'enable_fuse_generator', False):
        trainer.smirk_generator = DDP(
            trainer.smirk_generator,
            device_ids=[local_rank],
            output_device=local_rank,
            find_unused_parameters=find_unused_parameters,
            broadcast_buffers=broadcast_buffers,
...
if __name__ == '__main__':
    ...
    try:
        ...
        with diagnostics.stage('trainer.create_base_encoder', rank=rank):
        ...
        for epoch in range(config.train.resume_epoch, config.train.num_epochs):
                ...
                for batch_idx, batch in progress:
                    ...
                    if log_this_batch:
                        diagnostics.record_event(
                            ...
                            loss_first_path=reduced_loss.get('loss_first_path'),
                            loss_second_path=reduced_loss.get('loss_second_path'),
                            rank=rank,
                        )
            ...
            if is_main_process:
                ...
                if epoch % config.train.save_every == 0 or epoch == (config.train.num_epochs - 1):
                    ...
                    trainer.save_model(None, checkpoint_path)
                    diagnostics.record_event(
                        'checkpoint_save_end',
                        epoch_idx=epoch,
                        save_path=checkpoint_path,
